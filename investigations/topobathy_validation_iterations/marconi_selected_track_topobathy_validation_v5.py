...
def sample_dem_polygon(
    ...
    # Evaluate the 2-D grid of DEM cell centers directly.
    # Matplotlib Path.contains_points returns a flat boolean vector.
    # Reshape it explicitly to the DEM window shape before combining
    # it with the finite-data mask.
    ...
    expected_size = dem_shape[0] * dem_shape[1]

    if inside_flat.size != expected_size:
        raise RuntimeError(
            "Point-in-polygon result size does not match DEM window: "
            f"points={inside_flat.size}, DEM={dem_shape}"
        )
