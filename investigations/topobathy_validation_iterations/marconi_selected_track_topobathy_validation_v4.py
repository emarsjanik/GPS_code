...
def sample_dem_polygon(
    ...
    """
    Estimate wet fraction within an ellipse using all 1-m DEM cells
    whose centers fall inside the polygon.

    This is intentionally area/grid based rather than polygon-only:
    the water fraction is based on actual DEM elevations.
    """
    ...
    row0, col0 = raster.index(
        min_e,
        max_n,
    )
    row1, col1 = raster.index(
        max_e,
        min_n,
    )
    ...
    # Rasterio expects a Window(col_off, row_off, width, height).
    # The previous version passed four positional slice-like values in
    # the wrong form, which produced:
    #   Window.from_slices() got multiple values for argument 'height'
    ...
    # Pixel centers for north-up affine transform.
    rows_idx = np.arange(
        rmin,
        rmax + 1,
    )
    ...
    # Ellipse polygon point-in-polygon test using matplotlib Path.
    ...
    # Evaluate the 2-D grid of DEM cell centers directly.  The previous
    # version flattened the point coordinates but then paired the result
    # with the 2-D DEM array, causing:
    #   operands could not be broadcast together
    #
    # Keep the boolean polygon mask in the same (rows, cols) shape as data.
    ...
    if data.shape != inside.shape:
        raise RuntimeError(
            "DEM window and polygon mask shapes differ: "
            f"DEM={data.shape}, mask={inside.shape}"
        )
    ...
    n_cells = int(
        np.sum(footprint_mask)
    )

    # Approximate footprint total using ellipse polygon area and finite
    # DEM coverage from grid cells.
    valid_area_m2 = (
        n_cells
        * abs(
            raster.res[0]
